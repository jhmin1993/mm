...
```mermaid
flowchart TD
    subgraph GR["PO 참조 입고 (Mvt 101)"]
        direction LR
        G1["차변: 재고 계정 (BSX)<br/>재고 자산 증가"]
        G2["대변: GR/IR 정산 계정 (WRX)<br/>미결 채무 계상"]
    end
    subgraph IV["송장 검증 (MIRO)"]
        direction LR
        I1["차변: GR/IR 정산 계정 (WRX)<br/>정산"]
        I2["대변: 채무 계정 (Vendor)<br/>지급 채무"]
    end
    GR --> IV
```
